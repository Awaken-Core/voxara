...
const adapter = new PrismaPg({ connectionString: parsedEnv.DATABASE_URL });
...
// (your metadata object stays unchanged)
...
async function uploadToUploadThing({
  ...
  return response.data; // contains key + url
}

async function seedSystemVoice(name: string) {
  const { buffer, fileName, contentType } =
    await readSystemVoiceAudio(name);
  ...
  const uploadedFile = await uploadToUploadThing({
    buffer,
    fileName,
    contentType,
  });
  ...
  if (existingSystemVoice) {
    await prisma.voice.update({
      ...
      data: {
        r2ObjectKey: uploadedFile.key,  // 🔥 store UploadThing key
        ...(meta && {
