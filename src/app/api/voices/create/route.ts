...
export async function POST(request: Request) {
    ...
    try {
        ...
        const fileParseBuffer = Buffer.from(fileBuffer);
        const upload = await uploadAudiofile(fileParseBuffer)

        const r2ObjectKey = upload?.key || "";
        ...
        return Response.json(
            { error: `Failed to create voice. Please retry ${e}` },
            { status: 500 },
        );
    }
